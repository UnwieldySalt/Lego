...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//Error checking the Contact us form
var submit_button = document.getElementById("submit");
submit_button.disabled = true;  //This makes the submit button greyed out by default.

let a = 0;   //a variable that I make 1 if all the conditions are met and the user clicked the submit button. 
if(a == 1){  //when a = 1, the submit button is disabled again, hence it loops forever.
...
function myFunction() {
    ...
    let x = document.getElementById("numb").value; //x = Name (has to be text)
    let y = document.getElementById("text1").value; // y = email (error checked by the email tag itself)
    let z = document.getElementById("text2").value; //z = phone number(has to be a number)

    let text;  //this will be printed in the error_check id paragraph in the html file.

    if (isNaN(x)) {                     //if x(phone number) is Not a number (isNAN == true) then text would be "Input is not valid"
      text = "Input(s) not valid.";
    }
    else if(!isNaN(y) || /\d/.test(y)){ //if y(name) is a number(isNAN == false) and if any part of it is a number (/\d/.test(y) == true) then the text would be "it is not valid"
        text = "Input(s) not valid." 
    }
    else if(y.length>0 && x.length>0 && z.length == 0 || z.length >0 && x.length >0 && y.length==0){  // if the user didn't complete the form 
        text = "Please fill the form completely."
    }
    else if(x.length == 10 && y.length <1 && z.length <1){  //if the user filled the phone number and nothing else
        text = "Please fill the form completely"
    }
    else if(y.length >0 && z.length > 0 && x.length>1 && x.length<10 || x.length > 1 && x.length <10){  //if the user input an incomplete phone number.
        text = "Incomplete phone number." 
    }
    else if(x.length == 0){     //if the user didn't put anything anywhere.
        text = "Please fill the form completely.";
    }
    else if(y.length>0 && z.length>0 && x<1){    //if the user didn't input a phone number.
        text = "Enter a phone number."
    }
   
    else{
      submit_button.disabled = false;        //if everything above is not the case, i.e all the submissions were good (besides the email that is checked by the email input type)
      text = "Great, now you can submit."    //then the submit button is enabled, and a = 1 (so as soon as you submit, it goes back to being disabled for the next time).
      a = 1;
    }

    document.getElementById("error_check").innerHTML = text;  //the text(error check messages) are spit out through the paragraph with the id "error_check" of the associated html file.
  }
